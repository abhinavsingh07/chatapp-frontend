/**
 * MediaUploader.js
 * Handles media uploads (chat attachments, profile pictures) via S3 pre-signed URLs.
 *
 * Usage:
 *   const uploadId = MediaUploader.generateClientUploadId();
 *   const { valid, error, mediaType } = MediaUploader.validateSelectedFile(file, 'CHAT_ATTACHMENT');
 *   const { mediaId, uploadUrl } = await MediaUploader.initMediaUpload('CHAT_ATTACHMENT', uploadId, { file, conversationId });
 *   await MediaUploader.uploadFileToS3(uploadUrl, file, onProgress);
 *   const result = await MediaUploader.completeMediaUpload(mediaId, uploadId);
 *   MediaUploader.showUploadError('Something went wrong', 'chat');
...
class MediaUploader {
     ...
     * @param {string} usageType - 'CHAT_ATTACHMENT' or 'PROFILE_PICTURE'
     * @returns {{valid: boolean, error: string|null, mediaType: string|null}}
     */
    static validateSelectedFile(file, usageType) {
        if (!file) {
            return { valid: false, error: 'No file selected', mediaType: null };
        }

        // Profile picture: image only
        if (usageType === 'PROFILE_PICTURE') {
            if (!MediaUploader.CONFIG.ALLOWED_IMAGE_TYPES.includes(file.type)) {
                return { valid: false, error: 'Only JPEG, PNG, WebP, GIF, and BMP images are allowed for profile picture', mediaType: null };
            }
            if (file.size > MediaUploader.CONFIG.MAX_IMAGE_SIZE) {
                const sizeMB = (MediaUploader.CONFIG.MAX_IMAGE_SIZE / 1024 / 1024).toFixed(0);
                return { valid: false, error: `Image must be smaller than ${sizeMB} MB`, mediaType: null };
            }
        ...
        if (usageType === 'CHAT_ATTACHMENT') {
            ...
            if (MediaUploader.CONFIG.ALLOWED_IMAGE_TYPES.includes(file.type)) {
                mediaType = 'IMAGE';
                maxSize = MediaUploader.CONFIG.MAX_IMAGE_SIZE;
            } else if (MediaUploader.CONFIG.ALLOWED_VIDEO_TYPES.includes(file.type)) {
                mediaType = 'VIDEO';
                maxSize = MediaUploader.CONFIG.MAX_VIDEO_SIZE;
            } else if (MediaUploader.CONFIG.ALLOWED_DOCUMENT_TYPES.includes(file.type)) {
                mediaType = 'DOCUMENT';
                maxSize = MediaUploader.CONFIG.MAX_DOCUMENT_SIZE;
            } else {
                return { valid: false, error: 'File type not supported. Allowed: images, videos, or documents', mediaType: null };
            }

            if (file.size > maxSize) {
                const sizeMB = (maxSize / 1024 / 1024).toFixed(0);
                return { valid: false, error: `${mediaType} must be smaller than ${sizeMB} MB`, mediaType: null };
            }

            return { valid: true, error: null, mediaType };
        }

        return { valid: false, error: 'Invalid usage type', mediaType: null };
    }
     ...
     * @returns {Promise<{mediaId: string, uploadUrl: string, uploadUrlExpiresIn: number}>}
     */
    static initMediaUpload(usageType, clientUploadId, payload = {}) {
        return new Promise((resolve, reject) => {
            ...
            MediaUploader._debugLog(`[initMediaUpload] Calling ${uploadInitUrl} with clientUploadId=${clientUploadId}`);

            ajaxRequest(uploadInitUrl, 'POST', requestBody,
                function (response) {
                    MediaUploader._debugLog('[initMediaUpload] Success response:', response);
                    if (response && response.data && response.data.length > 0) {
                        const data = response.data[0];
                        MediaUploader._debugLog('[initMediaUpload] Received mediaId:', data.mediaId);
                        resolve({
                ...
                function (xhr, status, error) {
                    MediaUploader._debugLog('[initMediaUpload] Error:', status, error, xhr);
     ...
     * @param {Function|null} onProgress - Callback for progress updates {progress: 0-100, state: string}
     * @returns {Promise<void>}
     */
    static uploadFileToS3(uploadUrl, file, onProgress = null) {
        return new Promise((resolve, reject) => {
            MediaUploader._debugLog('[uploadFileToS3] Starting upload to S3');
            ...
            xhr.upload.addEventListener('progress', (event) => {
                if (event.lengthComputable) {
                    ...
                    MediaUploader._debugLog(`[uploadFileToS3] Progress: ${percentComplete}%`);
                    if (typeof onProgress === 'function') {
            ...
            xhr.addEventListener('load', () => {
                if (xhr.status === 200 || xhr.status === 204) {
                    MediaUploader._debugLog('[uploadFileToS3] Upload successful (status ' + xhr.status + ')');
                    if (typeof onProgress === 'function') {
                        onProgress({ progress: 100, state: 'Upload complete' });
                    }
                    resolve();
                } else {
                    MediaUploader._debugLog('[uploadFileToS3] Upload failed (status ' + xhr.status + ')');
            ...
            xhr.addEventListener('abort', () => {
                MediaUploader._debugLog('[uploadFileToS3] Upload aborted');
     ...
     * @returns {Promise<{mediaId: string, status: string, message: string}>}
     */
    static completeMediaUpload(mediaId, clientUploadId) {
        return new Promise((resolve, reject) => {
            if (!mediaId) {
            ...
            MediaUploader._debugLog(`[completeMediaUpload] Calling ${uploadCompleteUrl} for mediaId=${mediaId}`);

            ajaxRequest(uploadCompleteUrl, 'POST', requestBody,
                function (response) {
                    MediaUploader._debugLog('[completeMediaUpload] Success response:', response);
                    if (response && response.data && response.data.length > 0) {
                        const data = response.data[0];
                        resolve({
                            mediaId: data.mediaId,
                ...
                function (xhr, status, error) {
                    MediaUploader._debugLog('[completeMediaUpload] Error:', status, error, xhr);
     ...
     * @returns {Promise<{mediaId: string, status: string, message: string}>}
     */
    static retryUploadComplete(mediaId, clientUploadId) {
        MediaUploader._debugLog('[retryUploadComplete] Retrying completion for mediaId=' + mediaId);
        return MediaUploader.completeMediaUpload(mediaId, clientUploadId);
    }
     ...
     */
    static showUploadError(message, container = 'chat') {
        MediaUploader._debugLog('[showUploadError] ' + message);
     ...
     */
    static showUploadSuccess(message, container = 'chat') {
        MediaUploader._debugLog('[showUploadSuccess] ' + message);
     ...
     * @param {string} container - 'chat' or 'profile'
     */
    static setUploadLoadingState(state, container = 'chat') {
        MediaUploader._debugLog('[setUploadLoadingState] ' + state + ' for container=' + container);
        MediaUploader.STATE.isUploading = state !== 'completed' && state !== 'failed';
        MediaUploader.STATE.targetContainer = container;
        ...
        if (sendButton) {
            sendButton.disabled = MediaUploader.STATE.isUploading;
        }
        if (attachButton && container === 'chat') {
            attachButton.disabled = MediaUploader.STATE.isUploading;
        }
    }
     ...
     * @param {string} container - 'chat' or 'profile'
     */
    static resetUploadState(container = 'chat') {
        MediaUploader._debugLog('[resetUploadState] Resetting for container=' + container);

        MediaUploader.STATE.isUploading = false;
        MediaUploader.STATE.currentMediaId = null;
        MediaUploader.STATE.currentFile = null;
     ...
     * Logs only if MediaUploader.CONFIG.DEBUG is true
     * @param {...any} args - Arguments to log
     */
    static _debugLog(...args) {
        if (MediaUploader.CONFIG.DEBUG && typeof console !== 'undefined' && console.log) {
