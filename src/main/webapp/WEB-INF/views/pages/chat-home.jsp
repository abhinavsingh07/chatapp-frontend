               ...
               <%-- Sidebar header — desktop only, mobile uses ch-mobile-header above --%>
                  <div class="ch-sidebar-header d-none d-md-flex align-items-center justify-content-between">
                     ...
                     <%-- Protected: href URL mappings --%>
                        <div class="dropdown">
                           <button class="ch-icon-btn btn" type="button" data-bs-toggle="dropdown"
                              aria-expanded="false">
                  ...
                  <%-- Search bar. Protected: id="chatSearch" used by JS input listener --%>
                     <div class="ch-search-wrap">
                        <div class="input-group">
                           ...
                           <input type="text" class="form-control ch-search-input" placeholder="Search chats..."
                              id="chatSearch">
                        </div>
                     </div>
                           ...
                           <c:if test="${not empty chatData}">
                              <c:forEach var="data" items="${chatData}">
                                 <%-- Protected: .chat-item (JS querySelectorAll), data-touserid, data-conversationid
                                    --%>
                                    <div class="chat-item ch-item" data-touserid='<c:out value="${data.userId}"/>'
                                                ...
                                                <c:otherwise>
                                                   <c:choose>
                                                      <c:when test="${not empty data.mediaId}">
                                                         <div data-user-media-id="${data.mediaId}"
                                                            data-user-id="${data.userId}" data-profile-picture="true">
                                                         </div>
                                                      </c:when>
                                                      <c:otherwise>
                                             ...
                                             <div class="ch-item-body">
                                                <div class="ch-item-top">
                                                   ...
                                                   <%-- Protected: .sentAt class + data-date attribute used by JS --%>
                                                      <c:if test="${not empty data.content}">
                                                         <small class="ch-item-time sentAt"
                                                            data-date='<c:out value="${data.sentAt}"/>'></small>
                                                      </c:if>
                                                </div>
                                                <%-- Protected: p inside .chat-item used by JS querySelector('p') --%>
                                                   <c:choose>
                                                         ...
                                                         <p class="ch-item-preview">
                                                            <c:choose>
                                                               <c:when test="${data.senderId == userid}">
                                                                  <i class="fas fa-check text-primary me-1"
                                                                     style="font-size:0.68rem;"></i>You:
                                                               </c:when>
                                                               <c:otherwise>
                                                                  <i class="fas fa-check text-primary me-1"
                                                      ...
                                                      <c:otherwise>
                                                         <p class="ch-item-preview ch-item-preview--empty">No messages
                                                            yet</p>
                                                      </c:otherwise>
                                                   </c:choose>
                                             </div>
